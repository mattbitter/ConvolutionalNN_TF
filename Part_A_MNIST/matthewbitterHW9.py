#Matthew Bitter #CS498 - HW9 #MCS-DS
#May 7th, 2018
#List of changes:
 #line 36 Batch size to 2100
 #line 143 Added 3rd convolution
 #line 252 Replaced tf.estemator.train with tf.train_and_evaluate to run eval with training
 #line 214 Added config object to support train_and_evaluate

#REFERENCES: Big thank you to this github (nealwu) for details on how to implement tensorboard tracking
#using tf.layers architecture. nealwu selected AdamOptimizer and I agree that it is better for this v.s. SGD
...
def main(unused_argv):
  #config variable that controls how often to save checkpoint to be used by eval function
  ...
  mnist_classifier = tf.estimator.Estimator(
      model_fn=mnist_model_fn, config=run_config)
  ...
  #Matthew Bitter edit , May 7th, 2018
  ############################################
  #The code above I commented out and replaced with tf.train_and_evaluate function which runs the train and eval
  #at alternating times. currently it runs eval every 25 seconds which is more frequently than 100 steps
  train_spec = tf.estimator.TrainSpec(input_fn=lambda: input_fn(tf.estimator.ModeKeys.TRAIN, FLAGS.batch_size), max_steps=FLAGS.steps)
  eval_spec = tf.estimator.EvalSpec(input_fn=lambda: input_fn(tf.estimator.ModeKeys.EVAL, FLAGS.batch_size), steps=100, start_delay_secs=0,throttle_secs=25)
